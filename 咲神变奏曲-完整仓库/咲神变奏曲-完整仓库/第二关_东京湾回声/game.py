...
# 游戏引擎 - AI可玩版（修复版 v1.1）
# 修复清单：
#   1. 新增模块级 cmd()/new_game() 入口
#   2. 修复 look 瓦片 / look 潮位记录本 的 KeyError（补初始化计数器）
#   3. 补全 19 条设计线索的获取路径（场景观察 + NPC 对话），think 不再永远为空
#   4. NPC 增加位置校验与对话轮次推进；教授终章五段对话落地
#   5. 新增 choose 指令与三个结局（坏结局·被潮水吞没 / 普通结局·封印的记忆 / 好结局·潮声的回响）
#   6. 心智系统接线：抗拒触发 -15，心智归 0 直接进入坏结局；时间随移动推进，100% 触发终章催促
#   7. 塑料布在和管理员交谈后正确解锁
#   8. 设定自洽化：「潮声=三浦」明写；玄岳之名跨时代重现改为终章明示的悬念钩子（第三章伏笔）
