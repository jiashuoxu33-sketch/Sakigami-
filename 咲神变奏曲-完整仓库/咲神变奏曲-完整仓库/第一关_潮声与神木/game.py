...
# 游戏引擎 - AI可玩版（修复版 v1.1）
# 修复清单：
#   1. 新增模块级 cmd()/new_game() 入口 —— README 快速开始（import game; game.cmd(...)）现在可用
#   2. 修复 paint / look 画纸 的 ValueError 崩溃（画纸数量改为整数属性）
#   3. 新增 accuse 指令与三个结局（A 人犯之罪 / B 神堕之醒 / C 神堕结局），心智降至 0 自动触发结局 C
#   4. 信任值可增长（对话 +10，≥60 解锁深层证词），NPC 按场景就位，对话有轮次推进
#   5. 新增 search 指令与「井沿凹槽 → 打捞」路线，贝壳双路径（坠井 / 打捞）汇合
#   6. 经卷压痕改为二次 look 获得（与提示官指南一致）；线索/物品去重
#   7. 新增 look 贝壳 三次观察链（七芒星数字的正确解读：前六项等差+4，第七位本应是 27，却是 42）
#   8. 修复跨场景远程 talk/look；心智下限为 0；短刃/井口陷阱不会无限重复扣心智
#   9. 补全三位嫌疑人的矛盾点系统（think 自动汇总）
